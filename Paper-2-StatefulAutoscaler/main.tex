...
\item \textbf{Systematic sensitivity analysis (26 parameterized runs).} We vary each of the
  four primary operator-configurable parameters -- cooldown window, target connections per pod
  ($T$), Prometheus scrape interval, and \texttt{maxScaleDownStep} -- one at a time across the
  two-cycle restorm workload. All 26~runs preserve pod availability; the observed safe operating
  region of cooldown--gap combinations, together with four result tables and operator guidance rules,
  characterize the controller's practical parameter space within the evaluated range.
\end{enumerate}
...
the MQTT protocol generalization experiments. Section~\ref{sec:sensitivity} presents a
systematic sensitivity analysis of the four primary controller parameters.
Section~\ref{sec:limitations} discusses
...
(cooldown, step-rate) are enforced at the W\textit{single point} where the replica count
...
%% ============================================================
\section{Sensitivity Analysis: Parameter Space Characterization}
\label{sec:sensitivity}
%% ============================================================

The \texttt{StatefulAutoscaler} exposes four operator-configurable parameters whose
interaction determines the controller's safety envelope, convergence speed, and blast
radius. This section reports a systematic one-parameter-at-a-time sensitivity analysis
conducted over the two-cycle restorm workload (same topology as Experiment~C;
800~clients, 15-second Prometheus baseline). Each sweep fixes three parameters at their
default values (\texttt{scaleDownCooldownSeconds}~$= 120$, $T = 100$,
\texttt{scrape\_interval}~$= 15$~s, \texttt{maxScaleDownStep}~$= 2$) and varies the
fourth. A total of~26 parameterized runs were executed; raw results are available in the
repository under \texttt{results/raw/websocket/sensitivity/}.

\subsection{Test~1: Cooldown Window Sweep}

\textbf{Motivation.} The \texttt{scaleDownCooldownSeconds} parameter controls how
long the stabilization window holds pods warm after a connection drop.
Operators must set this value relative to the expected maximum duration of transient
disconnection gaps in their application.

\textbf{Protocol.} Six cooldown values (30, 60, 90, 120, 150, 180~s) were tested,
each with a connection gap 10~seconds shorter and 10~seconds longer than the
cooldown. The experimental design places test points immediately on either side of
the theoretical threshold $\text{gap} = \text{cooldown}$, evaluating behavior within a
10-second margin of the predicted transition point.

\begin{table}[ht]
\centering
\caption{Cooldown sweep results. All 12~runs preserved replicas through the
connection gap (safe~$= 1$). Scale-up time varied between 51--67~s.}
\label{tab:sens_cooldown}
\begin{tabular}{@{}rrrrrl@{}}
\toprule
\rowcolor{gray!12}
\textbf{Cooldown} & \textbf{Gap} & \textbf{Scale-up} & \textbf{$\Delta$ SD} & \textbf{SD dur.} & \textbf{Safe} \\
\textbf{(s)} & \textbf{(s)} & \textbf{(s)} & \textbf{(s)} & \textbf{(s)} & \\
\midrule
30 & 20 & 56 & 63 & 68 & \checkmark \\
30 & 40 & 61 & 68 & 78 & \checkmark \\
60 & 50 & 62 & 94 & 104 & \checkmark \\
60 & 70 & 67 & 93 & 108 & \checkmark \\
90 & 80 & 66 & 33 & 134 & \checkmark \\
90 & 100 & 51 & 126 & 136 & \checkmark \\
120 & 110 & 57 & 65 & 166 & \checkmark \\
120 & 130 & 56 & 154 & 164 & \checkmark \\
150 & 140 & 62 & 99 & 195 & \checkmark \\
150 & 160 & 61 & 181 & 196 & \checkmark \\
180 & 170 & 56 & 216 & 226 & \checkmark \\
180 & 190 & 51 & 215 & 225 & \checkmark \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Results.} Table~\ref{tab:sens_cooldown} reports the full sweep.
No unsafe transition was observed within the evaluated cooldown--gap combinations.
All tested configurations preserved pods through the connection gap, indicating
a stable operating region across the tested parameter range.

Figure~\ref{fig:safe_zone_boundary} visualises this result. The dashed line marks
the theoretical threshold $\text{gap} = \text{cooldown}$; all 12~points appear as
safe (green circles), including the six points where the gap \textit{exceeded} the
cooldown by 10~seconds, with no red crosses appearing anywhere in the plot.
A secondary observation is that scale-down duration increases linearly with cooldown
(from~68~s at $\text{CD} = 30$ to~226~s at $\text{CD} = 180$), confirming that
longer cooldowns impose a proportional de-provisioning delay after load removal.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{sensitivity/safe_zone_boundary.png}
\caption{Observed safe operating region of cooldown--gap combinations (Test~1).
All 12~evaluated points preserved replicas through the connection gap (green circles;
no unsafe transitions observed). The dashed line marks the theoretical threshold
$\text{gap} = \text{cooldown}$; the shaded region represents the expected safe zone
under the theoretical model. The absence of red crosses -- including at gap~$=$
cooldown~$+ 10$~s -- indicates the practical safety margin exceeds the theoretical
prediction within the tested range, but does not establish unconditional safety for
gap durations beyond the evaluated set.}
\label{fig:safe_zone_boundary}
\end{figure}

\begin{keyresult}
\noindent\textbf{Finding.} Across all evaluated cooldown values (30--180~s), the
controller maintained pod availability during transient connection gaps -- even when
the gap duration exceeded the configured cooldown by 10~seconds. These results
suggest the controller exhibits a wider practical safety margin than the theoretical
$\text{gap} = \text{cooldown}$ boundary. However, experiments were limited to the
tested parameter range and do not establish unconditional safety. \textbf{Operator
guidance}: set \texttt{scaleDownCooldownSeconds} to at least the maximum expected
disconnection gap plus one Prometheus scrape interval as a conservative lower bound.
\end{keyresult}

\subsection{Test~2: Target Connections per Pod ($T$)}

\textbf{Motivation.} The \texttt{targetConnectionsPerPod} parameter~$T$
determines the desired connection density and therefore the steady-state
replica count $\lceil N/T \rceil$. Operators must choose~$T$ based on their
pod memory limits and acceptable blast radius.

\textbf{Protocol.} Four values ($T = 50, 100, 150, 200$) were tested with
800~clients, yielding expected replica counts of 16, 8, 6, and~4.

\begin{table}[ht]
\centering
\caption{Target $T$ sweep: scale-up time, expected vs.\ peak replicas, and
theoretical blast radius ($\texttt{maxScaleDownStep} \times T$ connections
at risk per scale-down cycle).}
\label{tab:sens_target}
\begin{tabular}{@{}rrrrrr@{}}
\toprule
\rowcolor{gray!12}
\textbf{$T$} & \textbf{Expected} & \textbf{Peak} & \textbf{Scale-up} & \textbf{SD dur.} & \textbf{Blast} \\
 & \textbf{replicas} & \textbf{replicas} & \textbf{(s)} & \textbf{(s)} & \textbf{(conns)} \\
\midrule
50  & 16 & 18 & 71 & 179 & 100 \\
100 & 8  & 10 & 56 & 164 & 200 \\
150 & 6  & 7  & 56 & 159 & 300 \\
...
\textbf{Results.} Table~\ref{tab:sens_target} confirms that the controller
correctly scales to $\lceil 800/T \rceil$ replicas at every $T$ value tested,
with a transient overshoot bounded to~$+2$ replicas. Scale-up time
decreases from~71~s at $T = 50$ (16~pods to provision) to~31~s at $T = 200$
(only 4~pods needed). The trade-off is blast radius: at $T = 200$, each
scale-down step risks 400~connections versus only 100 at $T = 50$.
Figure~\ref{fig:t_sweep} visualises this inverse relationship.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{sensitivity/t_sweep.png}
\caption{Sensitivity to $T$: scale-up time decreases with larger~$T$ (fewer
pods to schedule), but blast radius increases proportionally. Operators must
balance provisioning speed against per-step connection exposure.}
\label{fig:t_sweep}
\end{figure}

\begin{keyresult}
\noindent\textbf{Operator guidance.} Choose $T$ to satisfy
$T \leq \text{pod\_memory\_limit} / \text{per\_connection\_overhead}$. For
workloads requiring minimal blast radius, use a smaller~$T$ (e.g.,~50) and
accept slower convergence; for latency-sensitive workloads with generous
memory, use a larger~$T$ (e.g.,~200) for faster provisioning.
\end{keyresult}

\subsection{Test~3: Prometheus Scrape Interval}

\textbf{Motivation.} The Prometheus scrape interval determines how quickly
the controller observes connection changes. Longer intervals delay both
scale-up and scale-down reactions.

\textbf{Protocol.} Three scrape intervals (15, 30, 60~s) were tested.

\begin{table}[ht]
\centering
\caption{Scrape interval sweep: scale-up reaction and scale-down reaction
times. Longer intervals degrade scale-down detection significantly.}
\label{tab:sens_scrape}
\begin{tabular}{@{}rrrr@{}}
\toprule
\rowcolor{gray!12}
\textbf{Scrape (s)} & \textbf{Scale-up (s)} & \textbf{SD reaction (s)} & \textbf{SD dur. (s)} \\
\midrule
15 & 61 & 78  & 169 \\
30 & 66 & 119 & 169 \\
60 & 66 & 190 & 200 \\
...
\textbf{Results.} Table~\ref{tab:sens_scrape} shows that scale-up reaction
is relatively stable (61--66~s), because it is dominated by pod scheduling
latency rather than metric freshness. However, scale-down reaction degrades
substantially: from~78~s at 15-second scrape to~190~s at 60-second scrape.
This is because the controller cannot detect that connections have dropped
until the next scrape delivers updated metrics. Figure~\ref{fig:scrape_sweep}
visualises both axes.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{sensitivity/scrape_sweep.png}
\caption{Sensitivity to Prometheus scrape interval: scale-up reaction is
dominated by pod scheduling and remains nearly flat; scale-down reaction
degrades proportionally with scrape interval.}
\label{fig:scrape_sweep}
\end{figure}

\begin{keyresult}
\noindent\textbf{Operator guidance.} A 15-second scrape interval is
recommended for production deployments. A 30-second interval is acceptable if
scale-down latency of $\approx$120~s is tolerable. A 60-second interval
should only be used when Prometheus resource constraints preclude faster
scraping, with the understanding that scale-down detection will be delayed
by up to 190~s.
\end{keyresult}

\subsection{Test~4: maxScaleDownStep}

\textbf{Motivation.} The \texttt{maxScaleDownStep} parameter bounds the
number of replicas removed per reconciliation cycle. It directly controls
the trade-off between de-provisioning speed and blast radius (connections at
risk per cycle).

\textbf{Protocol.} Four step sizes (1, 2, 3, 4) were tested.

\begin{table}[ht]
\centering
\caption{Step-size sweep: scale-down duration, observed maximum step-drop,
and theoretical blast radius ($\texttt{step} \times T$).}
\label{tab:sens_step}
\begin{tabular}{@{}rrrr@{}}
\toprule
\rowcolor{gray!12}
\textbf{Step} & \textbf{SD dur. (s)} & \textbf{Max drop} & \textbf{Blast (conns)} \\
\midrule
1 & 179 & 1 & 100 \\
2 & 164 & 2 & 200 \\
3 & 164 & 3 & 300 \\
...
\textbf{Results.} Table~\ref{tab:sens_step} confirms the expected trade-off.
Scale-down duration decreases modestly from~179~s at step~$= 1$ to~159~s at
step~$= 4$, while the theoretical blast radius quadruples. The observed
maximum step-drop matches the configured value (except at step~$= 4$, where
only~3 replicas were removed in a single cycle because the deployment was
already close to \texttt{minReplicas}). Figure~\ref{fig:step_sweep}
visualises the inverse relationship.

\begin{figure}[ht]
\centering
\includegraphics[width=\columnwidth]{sensitivity/step_sweep.png}
\caption{Sensitivity to \texttt{maxScaleDownStep}: scale-down duration
decreases with larger step sizes, but connections at risk per cycle increase
proportionally. Step~$= 2$ offers a balanced trade-off for the 800-client
workload.}
\label{fig:step_sweep}
\end{figure}

\begin{keyresult}
\noindent\textbf{Operator guidance.} Step~$= 1$ provides maximum safety but
slow convergence; step~$= 2$ is the recommended default, bounding blast
radius to $2T$ connections while converging in $\lceil(R_{\text{current}} -
R_{\text{desired}}) / 2\rceil$ cycles. Steps~$\geq 3$ should only be used
when fast de-provisioning is more important than connection safety.
\end{keyresult}

\subsection{Summary of Sensitivity Findings}

The four sweeps yield the following empirically defensible conclusions. First,
across all evaluated cooldown values (30--180~s), the controller maintained
pod availability during transient connection gaps, and no unsafe transition
was observed even when the gap duration exceeded the configured cooldown
by 10~seconds. This suggests the controller exhibits a wider practical safety
margin than the theoretical $\text{gap} = \text{cooldown}$ boundary predicts --
possibly because the sliding window retains high-water-mark entries from
the final seconds of CYCLE~1, providing residual protection beyond the nominal
cooldown. These results do not, however, establish unconditional safety for
gap durations outside the evaluated range.

Second, the scrape interval is a binding constraint on scale-down detection
latency: a 60-second scrape interval nearly triples scale-down reaction time
relative to 15~seconds, without affecting scale-up speed. Third, the $T$ and
\texttt{maxScaleDownStep} parameters expose an explicit speed-versus-blast-radius
trade-off that operators can audit via the formula
$\text{blast\_radius} = \texttt{step} \times T$ connections per scale-down cycle.
All 26~runs produced correct replica targeting within a bounded $+2$
overshoot, confirming the controller's formula-derived accuracy across the
