...
3{,}200 concurrent clients confirms linear replica targeting ($\lceil N/T \rceil + 2$
maxReplicas, achieved peak within 2 replicas of the formula prediction), scale-up reaction
times of 66--86~s, and 100\% connection preservation through DROP~1 at every scale; the
Kind OS-level TCP ceiling is documented at $\approx$3{,}392 achieved connections for 3{,}200
attempted. Comparative baseline evaluation against HPA
...
\item \textbf{Scalability validation at 800, 1{,}600, and 3{,}200 clients.}
  ...
  confirming linear replica targeting ($\lceil N/T \rceil$ replicas achieved within 2~replicas
  of the formula prediction at every scale), scale-up reaction times of 66--86~s, and 100\%
  connection preservation through the 90-second DROP~1 gap. The Kind OS-level TCP ceiling
  is documented at $\approx$3{,}392 achieved connections for 3{,}200 attempted clients.
...
$\lceil N / T \rceil$; ``Peak replicas'' is the maximum observed during CYCLE~1. ``Safe''
indicates whether all replicas were held through the 90-second DROP~1 gap
(1~$=$~yes, 0~$=$~no). The achieved connection count at 3{,}200 clients documents the
Kind OS-level TCP ceiling.}
\label{tab:scalability}
\begin{tabular}{@{}rrrrrrrr@{}}
\toprule
\rowcolor{gray!12}
\textbf{Clients} & \textbf{Expected} & \textbf{Peak} & \textbf{Scale-Up} & \textbf{Peak} & \textbf{DROP~1} & \textbf{Scale-Down} & \textbf{Scale-Down} \\
\rowcolor{gray!12}
\textbf{(attempted)} & \textbf{Replicas} & \textbf{Replicas} & \textbf{(s)} & \textbf{Connections} & \textbf{Safe?} & \textbf{Reaction (s)} & \textbf{Total (s)} \\
\midrule
800   & 8  & 10 & 66 & 874   & \checkmark & 73  & 164 \\
1{,}600 & 16 & 18 & 66 & 1{,}820 & \checkmark & 87  & 178 \\
3{,}200 & 32 & 34 & 86 & 3{,}392 & \checkmark & 77  & 218 \\
\bottomrule
\end{tabular}
\end{table*}

\paragraph{Linear replica targeting.}
At every scale, the controller converged to a peak replica count within 2~replicas of the
formula prediction $\lceil N / T \rceil$: 10~vs.~8 at 800~clients, 18~vs.~16 at 1{,}600, and
34~vs.~32 at 3{,}200. The consistent $+2$ overshoot is attributable to discrete-step
metric-to-decision latency during the CYCLE~1 ramp: because connections arrive faster
than the 5-second reconciliation period can converge, the controller briefly overshoots
before the window stabilises. This is the same mechanism observed in the single-scale
five-run replication (Section~\ref{sec:exp_c_multi}).
...
\paragraph{OS-level TCP ceiling.}
At 3{,}200 attempted clients, the achieved peak connection count was 3{,}392 ---
106\% of the attempted count, indicating that the Kind single-node cluster's OS-level
TCP stack (ephemeral port range, \texttt{net.core.somaxconn}, \texttt{listen()}
backlog) did not constrain the WebSocket workload at this scale. This contrasts with the
MQTT experiments (Section~\ref{sec:mqtt_extension}), where only 339 of 1{,}000
attempted clients successfully connected --- confirming that the TCP ceiling is
protocol- and application-specific, not a fundamental Kind limitation.

Figure~\ref{fig:scalability_summary} visualises the three-panel scalability summary:
replica targeting accuracy, scale-up reaction time, and the OS TCP ceiling comparison.
...
targeting --- achieved peak replicas (blue) track the expected $\lceil N/T \rceil$ (dashed)
with a consistent $+2$ overshoot. Centre: scale-up reaction time increases modestly from
66~s to 86~s at $4\times$ the baseline client count. Right: achieved peak connections
(red) meet or exceed the attempted count at all scales, confirming no OS-level TCP
bottleneck for the WebSocket workload in the Kind environment.}
\label{fig:scalability_summary}
\end{figure*}

\begin{keyresult}
\noindent\textbf{Conclusion.} The \texttt{StatefulAutoscaler}'s connection-density formula
($\lceil N / T \rceil$) produces correct replica targeting across a $4\times$ range of
client counts (800 to 3{,}200). Scale-up reaction time degrades sub-linearly (66~s to 86~s),
connection preservation through the 90-second DROP~1 gap is maintained at every scale
(100\% safe), and no OS-level TCP ceiling was observed for the WebSocket workload.
These results confirm that the controller's architecture scales without parameter
re-tuning beyond adjusting \texttt{maxReplicas}.
\end{keyresult}
...
\textit{pod-seconds} $4{,}280 \pm 67$). Scalability validation at 1{,}600 and 3{,}200
concurrent clients confirmed linear replica targeting ($\lceil N/T \rceil$ within 2~replicas
at every scale), 66--86~s scale-up reaction times, and 100\% connection preservation through
DROP~1, with no OS-level TCP ceiling observed for the WebSocket workload.
