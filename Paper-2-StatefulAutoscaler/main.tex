...
\item \textbf{Scalability validation at 800, 1{,}600, and 3{,}200 clients.}
  ...
  re-tuning. The Kind OS-level TCP ceiling is documented at $\approx$3{,}392 achieved
  connections for 3{,}200 attempted clients.
...
\paragraph{Replica tracking with bounded transient overshoot.}
At every scale, the controller's peak replica count was within $+2$ of the steady-state
formula prediction $\lceil N / T \rceil$: 10 vs.\ 8 at 800~clients, 18 vs.\ 16 at
1{,}600, and 34 vs.\ 32 at 3{,}200. This consistent $+2$ transient overshoot is the
expected behaviour of a discrete-time controller operating under metric-reporting
latency: during the CYCLE~1 ramp, connections arrive faster than the 5-second
reconciliation period can converge, so the controller briefly overshoots before
stabilising at the formula value. Crucially, the overshoot is bounded, reproducible, and
does not grow with client count --- confirming that the controller tracks $\lceil N/T\rceil$
with predictable, finite error rather than exhibiting runaway provisioning. This is the
same transient mechanism documented in the single-scale five-run replication
(Section~\ref{sec:exp_c_multi}).
...
3{,}200), the \texttt{StatefulAutoscaler} tracks the theoretical replica target
$\lceil N / T \rceil$ with a bounded transient overshoot of exactly $+2$~replicas
at every scale --- a consistent, predictable artefact of discrete-time reconciliation
under connection-ramp conditions, not a control-law defect. Scale-up reaction time
degrades sub-linearly (66~s to 86~s), 100\% connection preservation through DROP~1
is maintained at every scale, and no OS-level TCP ceiling was encountered for the
WebSocket workload. No parameter re-tuning beyond adjusting \texttt{maxReplicas} was
required, confirming that the controller's architecture scales in practical proportion
to the client count.
\end{keyresult}
...
maintained with no parameter re-tuning and no OS-level TCP ceiling.
