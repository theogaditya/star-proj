...
\begin{keyresult}
\noindent\textbf{MQTT finding.} The MQTT extension validates the full lifecycle of connection-aware autoscaling with drain semantics: the controller reacted 44\% faster than HPA using the correct metric, achieved session redistribution via drain-reconnect (176/163/0 $\rightarrow$ 247/91/0), and under an identical forced 3$\rightarrow$1 scale-down scenario, preserved all 339 sessions with a maximum step-drop of 2 versus MQTT-A's 639-connection cliff. The 339-client ceiling is a transport-level broker capacity limitation that no reactive autoscaler can eliminate without predictive pre-provisioning.
\end{keyresult}
...
\item \textbf{Drain semantics scope.} Graceful connection draining was validated in the MQTT-B extension (Section~9.2) but has not yet been tested under WebSocket workloads. Production deployment requires coordination between the controller, load balancer, and application for each protocol.
...
The MQTT extension further validates the design by demonstrating drain-based session rebalancing under an identical forced scale-down scenario. Under MQTT, CPU-based HPA reacted 30~seconds too late and a forced 3$\rightarrow$1 scale-down severed approximately 300 live sessions (a 639-connection cliff with reconnection storm peaking at 837). The \texttt{StatefulAutoscaler} with drain semantics enabled scaled to $\lceil 339/150 \rceil = 3$ replicas 44\% faster ($t=38$~s versus $t=68$~s), redistributed sessions from 176/163/0 to 247/91/0 via controller-driven drain, and under the same forced 3$\rightarrow$1 reduction preserved all 339 sessions with a maximum step-drop of 2 (instrument noise). The 339-client ceiling in both experiments reflects a transport-level broker capacity constraint during the initial connection ramp, a limitation that no reactive autoscaler can eliminate without predictive pre-provisioning or client-side retry logic.
